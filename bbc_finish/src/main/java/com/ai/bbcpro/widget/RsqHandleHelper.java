...
//该类是网络请求类,利用的是httpclient但是client现在已经不用了
public class RsqHandleHelper {
    ...
    static private DefaultHttpClient connectServer(int rspCookie,
        ...
        // 如果是使用的运营商网络
        if (NetworkData.getNetworkInfo() != null && NetworkData.getNetworkInfo().getType() == ConnectivityManager.TYPE_MOBILE) {
            // 获取默认代理主机ip
    ...
    static private HttpUriRequest buildAndSendRsq(DefaultHttpClient connection,
                                                  ...
                                                  INetStateReceiver stateReceiver) throws IOException ,NullPointerException {
