...
public class MySelectableTextView extends androidx.appcompat.widget.AppCompatTextView {
    ...
    private MyOnWordClickListener mOnWordClickListener;
    ...
    public void setOnWordClickListener(MyOnWordClickListener onWordClickListener) {
        mOnWordClickListener = onWordClickListener;
    }

    public MySelectableTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void setText(CharSequence text, BufferType type) {
        ...
        if (isEnableSingleSelect) {
            ...
            // 分割单词
            List<MyWordInfo> wordInfos = getWordInfo();
            // 设置点击事件
            for (int i = 0; i < wordInfos.size(); i++) {
                MyWordInfo info = wordInfos.get(i);
     ...
     * @return
     */
    private List<MyWordInfo> getWordInfo() {
        // 获取分割之后的所有单词
        List<String> words = splitWord();
        // 创建WordInfos
        List<MyWordInfo> result = new ArrayList<>();
        int startIndex = 0;
    ...
    private List<String> splitWord() {
        ...
        // Pattern pattern = Pattern.compile("/\\b\\w+\\b/g");
