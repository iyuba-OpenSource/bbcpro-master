...
public class AudioRecorderUtils {
    ...
    private static AudioRecorderUtils m;

    public static AudioRecorderUtils getInstance() {
        synchronized (mLock) {
            if (m == null) {
                m = new AudioRecorderUtils();
            }
        }
        return m;
    }
    ...
    private AudioRecorderUtils() {
        ...
        this(MainApplication.application.getExternalFilesDir(Environment.DIRECTORY_MUSIC).getAbsolutePath()
                + "/record/");
    }
...
//            filePath = FolderPath + getNowTime() + ".3gp" ;
    ...
    public void UnBindStatusListener() {
